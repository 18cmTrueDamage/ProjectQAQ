...
public class Servicedb extends SQLiteOpenHelper {
    ...
    public Service findService2(String servicename, String rate){
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT * FROM "+TABLE_SERVICES+" WHERE "+COLUMN_SERVICENAME+" = \""+servicename+"\"" +" AND "+  COLUMN_RATE+" = \""+rate+"\"";
        Cursor cursor = db.rawQuery(query,null);
        Service service = new Service();
        if(cursor.moveToFirst()){
            service.setID(Integer.parseInt(cursor.getString(0)));
